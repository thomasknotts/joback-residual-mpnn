...
def one_hot_encoding(x, permitted_list):
    if x not in permitted_list:
        x = permitted_list[-1]
    return [int(b) for b in map(lambda s: x == s, permitted_list)]

def atom_features(atom, use_chirality=True, hydrogens_implicit=True):
    #permitted_list_of_atoms = ['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl', 'Br', 'Mg', 'Na', 'Ca', 'Fe', 'As', 'Al', 'I',
    #                           'B', 'V', 'K', 'Unknown']
                               
    permitted_list_of_atoms = ['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl', 'Br', 'I', 'Unknown']
...
def bond_features(bond, use_stereochemistry=True):
